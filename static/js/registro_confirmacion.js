...
async function iniciarAuditoria() {
  ...
  try {
    const response = await fetch("/lanzar-escaneo", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email, dominio, forzar: esReEscaneo }),
    });
    const data = await response.json();
...
function mostrarResumen(data) {
  ...
  const linkPrincipal = document.getElementById("linkDescarga");
  linkPrincipal.href = data.url_completa;

  let extra = document.getElementById("linksExtra");
  if (!extra) {
    extra = document.createElement("div");
    extra.id = "linksExtra";
    extra.style.marginTop = "10px";
    linkPrincipal.parentNode.insertBefore(extra, linkPrincipal.nextSibling);
  }

  const descargas = data.descargas || {};
  const partes = [];
  if (descargas.zap) partes.push(`<a href="${descargas.zap}" class="button">Descargar ZAP</a>`);
  if (descargas.burp) partes.push(`<a href="${descargas.burp}" class="button">Descargar Burp</a>`);
  extra.innerHTML = partes.join(" ");
}
